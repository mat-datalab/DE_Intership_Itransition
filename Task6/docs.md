...
Algorithm:
1. Compute initial state via prng_init(...).
2. Apply prng_next(state).
3. Return state / 2147483648.0.
...
1. Generate u1, u2 ~ Uniform(0,1) using two independent streams:
~~~~sql
u1 := rand_uniform_01(..., p_stream);
u2 := rand_uniform_01(..., p_stream + 1);
~~~~

2. Compute standard normal:
~~~~sql
...
1. Sample two uniforms:
~~~~sql
u1 := rand_uniform_01(..., p_stream);
u2 := rand_uniform_01(..., p_stream + 1);
~~~~

2. Use the standard algorithm for uniform sampling on a sphere:
~~~~sql
...
2. Draw uniform u and compute offset floor(u * cnt).
...
2. Random house number between 1 and 200.

3. Formatting depends on locale:
- en_US: house street, city postal_code
- de_DE: street house, postal_code city
...
1. Pick a random pattern from phone_formats(locale, pattern):
    ~~~~sql
...
2. Iterate over characters in v_pattern:
...
1. Read physical parameters for the locale:
~~~~sql
SELECT * INTO cfg
...
2. For each i from 0 to p_batch_size - 1:
...
3. Return each constructed fake_user row.
